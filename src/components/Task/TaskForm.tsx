...
import { Task } from '../../types';
  ...
  const TaskForm: React.FC<{ task: Task | null, sendFormData: () => void }> = ({
    task,
    sendFormData,
  }) => {
    ...
    // const statusOptins = !statuses.length ? [task?.status] : statuses;
  
    form.setFieldsValue({ status: task?.status });
    const onCreateProject = (values: any) => {
      const params = { authorId, ...task, ...values };
